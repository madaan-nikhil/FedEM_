...
if __name__ == "__main__":
    ...
    for itt in range(len(exp_names)):
        ...
        args_.seed = 1234
        args_.verbose = 1
        args_.save_path = 'weights/cifar10/FedAvg_replacement/FAT_benign_fedavg/' + exp_names[itt]
        args_.validation = False
        args_.save_freq = 20
        args_.tune_steps = 0
        args_.aggregation_op = 'krum'

        # Other Argument Parameters
        Q = 10 # update per round
        G = 0.5
        num_clients =  40 #original value 40
        S = 0.05 # Threshold
        step_size = 0.01
        K = 10
        eps = 0.1
        ...
        for i in range(atk_count):
            aggregator.clients[i].turn_malicious(
                ...
                replace_model_path = "weights/cifar10/FedAvg/chkpts_0.pt"
            )
