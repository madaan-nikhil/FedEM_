...
class Transferer(): 
    ...
    def __init__(self, models_list, dataloader):
        ...
        self.x_orig_fixed = None
        self.y_orig_fixed = None
    ...
    def generate_xadv(self, load_single_batch=False, atk_type = "IFSGM", mode='test'):
        ...
        if self.x_orig_fixed is None:
            self.x_orig_fixed = self.advNN.x_orig.clone()
            self.y_orig_fixed = self.advNN.output_orig.clone()
    ...
    def check_empirical_metrics(self, orig_flag = True, batch_size = 1000):
        ...
        if orig_flag: # Split between fooled and not fooled
            ...
            self.metric_variance = calcNN_variance(self.advNN, self.x_orig_fixed, self.y_orig_fixed)
            self.metric_loss = calcNN_loss(self.advNN, self.x_orig_fixed, self.y_orig_fixed)
            # For robust data
            for i in self.victim_idxs:
                # data_x = self.x_orig[self.robust_indices[i]]
                # data_y = self.y_orig[self.robust_indices[i]]
                data_x = self.x_orig_fixed
                data_y = self.y_orig_fixed
            ...
            data_x, data_y = self.x_orig_fixed, self.y_orig_fixed
            ...
            self.metric_loss = calcNN_loss(self.advNN, data_x, data_y)
